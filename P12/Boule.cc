...
Boule::Boule(double r=0, double m=0, double frt=0, double rest=0, double roul=0, double glis=0, Support_a_dessin* vue=nullptr)
	: 	Objet(vue),
		rayon(r),
		masse(m),
		coeff_frt(frt),
...
void Boule::setVitesse_Angulaire(Vecteur const& A) {
	...
	vector<double> tab(A.getV());
	derivee_temp.modifie(4, tab[0]);
	derivee_temp.modifie(5, tab[1]);
	derivee_temp.modifie(6, tab[2]);
	
}

void Boule::setAltitude(double const& a) { etat.modifie(3, a); }

void Boule::setVerticale_vitesse(double const& v) { derivee_temp.modifie(3, v); }

void Boule::setVerticale_force(double const& f) { force.modifie(3, f); }

/*---------------------------------------------------------------------------*/

// fonction affiche et dessine

ostream& Boule::affiche(ostream& sortie) const {
	
	sortie << "Masse : " << getmasse() << endl
		   << "Rayon : " << getrayon() << endl
		   << "Masse volumique : " << masse_volumique()  << endl
		   << "Position : "  << "(" << getEtat().getV()[0] << ", " << getEtat().getV()[1] << ", " << getEtat().getV()[2] << ")" << endl
		   << "Angles : " << "(" << getEtat().getV()[3] << ", " << getEtat().getV()[4] << ", " << getEtat().getV()[5] << ")" << endl
		   << "Vitesse : " << "(" << getDerivee().getV()[0] << ", " << getDerivee().getV()[1] << ", " << getDerivee().getV()[2] << ")" << endl 
		   << "Vitesse angulaire : " << "(" << getDerivee().getV()[3] << ", " << getDerivee().getV()[4] << ", " << getDerivee().getV()[5] << ")" << endl 
		   << "Force : " << getForce() << endl
		   << "Coefficient de restitution : " << getresti() << endl
		   << "Coefficient de frottement : " << getcoef() << endl
		   << "Coefficient de frottement (roulement) : " << getroul() << endl
		   << "Coefficient de frottement (glissement) : " << getglis() << endl
		   << endl;
...
void Boule::dessine() { support->dessine(*this); }

/*---------------------------------------------------------------------------*/

// autres methodes

void Boule::supprime() { delete this; }
...
double quadratique(double const& A, double const& B, double const& C) {
	...
	if(delta < 0) resultat = -1;
	else if(delta == 0) {
		resultat = -B/(2*A);
		if(resultat<0 or resultat>1) resultat = -1;
	}
	else {
...
double Boule::t_collision(Boule& B2, double const& t, double const& dt, Integrateur* I) const {
